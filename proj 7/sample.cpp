...
int   uLevel = 1;
float uQuantize = 50.f;
float uSize = 0.01f;
float uKa = 0.05f;
float uKd = 0.6f;
float uKs = 0.5f;
float uShininess = 10.f;
...
Animate( )
{
	...
	//uLevel = (int)((sin(2.f * F_PI * Time) * 0.5f + 0.5f) * 3.f);
	//uQuantize = 2.f + (sin(2.f * F_PI * Time * 2.f) * 0.5f + 0.5f) * 48.f;
	//uSize = 0.0001f + (sin(2.f * F_PI * Time * 3.f) * 0.5f + 0.5f) * 0.0999f;
...
Display()
{
	...
	Pattern.Use();
	Pattern.SetUniformVariable("uLevel", uLevel);
	Pattern.SetUniformVariable("uQuantize", uQuantize);
...
InitGraphics()
{
	...
	Pattern.Init();
	bool valid = Pattern.Create((char*)"pattern.vert", (char*)"pattern.frag", (char*)"pattern.geom");
...
InitLists( )
{
	...
	glPushMatrix();                 
	LoadObjFile((char*)"obj_files/deer.obj");     
	glPopMatrix();
	glEndList();
...
Keyboard( unsigned char c, int x, int y )
{
	...
	switch( c )
	{
		...
		case 'z': uLevel = clamp(uLevel - 1, 0, 3);      break;
		case 'Z': uLevel = clamp(uLevel + 1, 0, 3);      break;

		case 'x': uQuantize = clamp(uQuantize - 5.f, 2.f, 50.f);   break;
		case 'X': uQuantize = clamp(uQuantize + 5.f, 2.f, 50.f);   break;

		case 'c': uSize = clamp(uSize - 0.001f, 0.0001f, 0.1f);   break;
		case 'C': uSize = clamp(uSize + 0.001f, 0.0001f, 0.1f);   break;
