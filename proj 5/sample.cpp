...
//float uSc = 0.0f;
//float uTc = 0.0f;
//float uRad = 0.25f;      // give a sensible default radius
//float uMag = 0.0f;    // shader reads this; set by toggle
//float uWhirl = 0.0f;    // shader reads this; set by toggle
//float uMosaic = 0.0f;    // shader reads this; set by toggle
//int tMagnify = 0;
//int tWhirl = 0;
//int tMosaic = 0;
//int tOutline = 1;        // keep the ring visible by default
//const float MAX_MAG = 8.0f;    // strong zoom
//const float MAX_WHIRL = 30.0f;   // degrees at rim
//const float MAX_MOSAIC = 0.05f;   // block size in (s,t)
//float uOutlineWidth = 0.006f;
//float uOutlineColor[3] = { 1.0f, 0.0f, 0.0f };  // red
...
Display()
{
	...
	glBindTexture(GL_TEXTURE_2D, JinxTexture);
	Pattern.SetUniformVariable("uImageUnit", 0);     // sampler -> unit 0

	Pattern.SetUniformVariable("uSc", uSc);
	Pattern.SetUniformVariable("uTc", uTc);
	Pattern.SetUniformVariable("uRad", uRad);
	Pattern.SetUniformVariable("uMag", uMag);
	Pattern.SetUniformVariable("uWhirl", uWhirl);
	Pattern.SetUniformVariable("uMosaic", uMosaic);
	// ---------------------------------
...
Keyboard( unsigned char c, int x, int y )
{
	...
	switch( c )
	{
		...
		case 'z': uRad = fmaxf(0.01f, uRad - 0.01f); break; // smaller
		case 'Z': uRad = fminf(0.75f, uRad + 0.01f); break; // larger

		case 'x': uSc = fmaxf(0.0f, uSc - 0.02f); break;   // move lens LEFT
		case 'X': uSc = fminf(1.0f, uSc + 0.02f); break;   // move lens RIGHT

		case 'c': uTc = fmaxf(0.0f, uTc - 0.02f); break;   // move lens DOWN
		case 'C': uTc = fminf(1.0f, uTc + 0.02f); break;   // move lens UP

		case 'v': uMag = 1.0f; break;  // OFF
		case 'V': uMag = 2.0f; break;  // ON

		case 'b': uWhirl = 0.0f; break;  // OFF
		case 'B': uWhirl = 20.0f; break; // ON

		case 'n': uMosaic = 0.0f; break; // OFF
		case 'N': uMosaic = 0.03f; break; // ON
